class AttendanceGridPresenter
  ...
  def hours_label(date)
    hrs = attendance_hours(date)
    unit = (hrs == 1 ? "hour" : "hours")
    "#{hrs} #{unit}"
  end
  ...
  def config_start
    ...
    str = config["_config"]["start_date"] rescue nil
  ...
  def config_end
    return nil unless config_present?
    str = config["_config"]["end_date"] rescue nil
    return nil if str.blank?
  ...
  def config_present?
    config.respond_to?(:[]) && config["_config"].is_a?(Hash)
  end
