	...
	def links_to_ul(list)
		...
		list.each do |item, content|
			if content.is_a?(Hash)
				...
				items << content_tag(:li, toggle_progress_form(content) + link_to(insert_badge(item), content, remote: false, class: "nav-link py-2 flex-fill rounded-right", style: "padding-left: 0.25rem !important;", target: content =~ /^http/i ? '_blank' : ''), class: "nav-item bg-light p-0 rounded mb-1 d-flex align-items-center")
			elsif content.nil?
